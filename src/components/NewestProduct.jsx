      ...
      <div className='mb-56'>
         ...
         <div className='flex justify-between text-2xl mb-7 '>
            ...
            <ul className='gap-4 flex text-sm'>
               <li className={`cursor-pointer  ${userWantProduct === "هدفون" ? "border-b border-[#1462cf]" : "" }`} onClick={() => { changeCategory("هدفون") }}>هدفون</li>
               <li className={`cursor-pointer  ${userWantProduct === "موس" ? "border-b border-[#1462cf]" : "" }`} onClick={() => { changeCategory("موس") }}>موس</li>
               <li className={`cursor-pointer  ${userWantProduct === "مانیتور" ? "border-b border-[#1462cf]" : "" }`} onClick={() => { changeCategory("مانیتور") }}>ماننیتور</li>
            </ul>
         </div>
                  ...
                  <div className='relative flex flex-col border border-zinc-300 p-4 rounded-xl group m-2 z-50 transition-all duration-500 overflow-hidden '>
                     ...
                     <div>
                        <div className='flex justify-between mb-4'>
                           <span className='text-zinc-500 text-xs'>{pro.category}</span>
                           {pro.score && (
                        ...
                        <div className='transition-all duration-500'>
                           <div className='mb-10 mt-5 h-11'>
                              <h2 className='text-[15px] line-clamp-2'>{pro.title}</h2>
                           </div>
                           <div className='text-left text-sm'>
                              <span>{pro.price.toLocaleString()} تومان</span>
                           </div>
                        </div>
                        ...
                        {pro.discount && (
                           <div className='w-22 h-6 flex text-[12px] text-white transform -rotate-90 absolute top-8 -right-2'>
                              <div className=' bg-emerald-700 px-2 py-1 '>{pro.discount}%</div>
